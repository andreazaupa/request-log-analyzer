...
module RequestLogAnalyzer
  ...
  class Controller
    ...
    def self.build_from_arguments(arguments)
      ...
      options[:log_file_id]       = arguments[:log_file_id]
    ...
    def self.build(options)
      ...
      if options[:file]
        ...
        { :output         => output_instance,
          ...
          :log_file_id       => options[:log_file_id],    
          :yaml           => options[:yaml],
        ...
        def run!
          ...
          @aggregators.each { |agg| 

            if options[:log_file_id]
              agg.prepare(:log_file_id => options[:log_file_id]) 
            else
              agg.prepare
            end
          }
